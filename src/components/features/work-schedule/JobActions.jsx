...
import { UserPlus, UserCog, Settings, DollarSign } from "lucide-react";
...
const JobActions = ({ 
  ...
  if (mode === "assign-only") {
    return (
      <div className={`flex flex-shrink-0 items-center space-x-1 sm:space-x-2 ${className}`}>
        <button
          onClick={(e) => handleAction(e, onAssign)}
          className="p-1 sm:p-2 text-gray-500 rounded-full transition-colors hover:text-brand-green hover:bg-brand-green/10"
          title="Phân công thợ"
        >
  ...
  // Mode "full" - hiển thị tất cả nút (trừ copy)
  return (
    <div className={`flex flex-shrink-0 items-center space-x-1 sm:space-x-2 ${className}`}>
