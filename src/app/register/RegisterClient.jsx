...
export default function RegisterClient() {
  ...
  const validate = () => {
    ...
    if (!form.code || isNaN(form.code) || +form.code < 0 || +form.code > 999)
      return "Code phải là số từ 0-999";
    ...
    if (!/^\d{8,15}$/.test(form.phone_business.trim()))
      return "Số công ty phải là số 8-15 chữ số";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 422) {
          const errorMessage =
            data.message || data.error || "Dữ liệu không hợp lệ";
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-br from-blue-100 to-purple-200">
      <form
        ...
        className="p-8 space-y-5 w-full max-w-md bg-white rounded-xl shadow-lg"
      >
        ...
        {success && (
          <div className="p-2 text-sm rounded text-brand-green bg-brand-green/10">
            {success}
          </div>
        )}
        <div>
